...
RSpec.describe JobIoWrapper, type: :model do
  ...
  let(:file_set) { instance_double(FileSet, id: file_set_id, uri: 'http://127.0.0.1/rest/fake/bn/99/96/72/bn999672v', import_url: nil) }
  ...
  describe 'uploaded_file' do
    ...
    context 'with remote file' do
      let(:file_set) { instance_double(FileSet, id: file_set_id, uri: 'http://127.0.0.1/rest/fake/bn/99/96/72/bn999672v', import_url: 'http://foo/download') }
      it 'extracts filename from http get' do
        allow(FileSet).to receive(:find).with(file_set_id).and_return(file_set)
        WebMock.stub_request(:get, file_set.import_url)
               .to_return(status: 200, body: "", headers: { "content-disposition" => ["attachment;filename=\"filename.jpg\";filename*=UTF-8''filename.jpg"] })
        expect(subject.original_name).to eq('filename.jpg')
      end
    end
