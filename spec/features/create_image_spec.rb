...
RSpec.describe 'Create a Image', :feature, js: true do
  context 'a logged in user', :clean_repo do
    ...
    it do
      ...
      select license, from: "image_license"

      # Non-required fields

      fill_in('Publisher (Required for DOI registration)', with: publisher)
      fill_in('Date Created', with: date_created)
      fill_in('image_alternate_title', with: alternate_title)
      select(genre, from: 'image_genre')
      ...
      expect(page).to have_field('image_title', with: 'My Test Work')
      expect(page).to have_field('image_creator', with: creator)
      expect(page).to have_field('image_college', with: college)
      expect(page).to have_field('Program or Department', with: program)
      expect(page).to have_field('Description', with: description)
      expect(page).to have_select('image_license', selected: license)
      ...
      expect(page).to have_field('image_alternate_title', with: alternate_title)
      expect(page).to have_select('image_genre', selected: genre)
